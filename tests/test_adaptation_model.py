...
committed snapshot config, processor and tokenizer (always), and the pinned checkpoint itself where its
weights are staged (local pre-flight). They cover a referenced evaluation, a one-epoch adaptation of the last
decoder block, the trainable-tensor scope, the artifact round trip, the loader's tensor-set check, the
transactional guarantee and — where CUDA is visible — the same path on the accelerator."""
...
from pix2struct_textcaps_pipeline import (  # noqa: E402  -- after the importorskip guards
    DEFAULT_WEIGHTS_DIR,
    WEIGHT_FILE,
...
def _tiny_pipeline(seed: int = 0) -> Pix2StructTextCapsPipeline:
    ...
    return Pix2StructTextCapsPipeline._from_model(model, processor, "cpu", "tiny-random")


def _real_pipeline(device: str = "cpu") -> Pix2StructTextCapsPipeline:
    return Pix2StructTextCapsPipeline.from_pretrained(device=device)
...
def _scene(path, colour, shape, word):
    image = Image.new("RGB", (256, 192), (135, 206, 235))
    draw = ImageDraw.Draw(image)
    draw.rectangle([0, 130, 256, 192], fill=(60, 179, 75))
    if shape == "circle":
        draw.ellipse([80, 30, 176, 126], fill=colour)
    else:
        draw.rectangle([80, 30, 176, 126], fill=colour)
    draw.text((96, 140), word, fill="white")
...
def records(tmp_path_factory):
    ...
    out = []
    for i in range(12):
        colour, shape, word = COLOURS[i % 4], "circle" if i % 2 else "square", f"SIGN {i}"
        path = _scene(root / f"scene{i}.png", colour, shape, word)
        out.append(
            {
                "id": f"s{i:02d}",
                "image_id": f"scene{i}",
                "image": str(path),
                "captions": [
                    f"A {colour} {shape} above a sign that says {word}.",
...
def _captions(pipe, records):
    ...
    for r in records:
        with Image.open(r["image"]) as image:
            image.load()
            out.append(pipe.caption(image, max_new_tokens=8)["caption"])
    return out
...
def test_caption_adapt_and_reload_run_on_a_cuda_device(records, tmp_path):
    ...
    reloaded = Pix2StructTextCapsPipeline.from_artifact(artifact, device="cuda:0")
